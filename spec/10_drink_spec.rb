...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      it 'is your choice of beverage' do
        expect(my_coke.type).to eql('coke')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_coke.ounces).to eql(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_ocha) { described_class.new('ocha') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(my_ocha).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_soju) {described_class.new('soju', 10)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_soju).not_to be_full
      end
    end
  end
end
