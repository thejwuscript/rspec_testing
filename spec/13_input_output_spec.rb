...
# Long methods are like a run-on sentence that should have been divided into 2 or 3 different sentences
# so that everything could be clearly understood and in this case if a method does many different things
...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      # remove the 'x' before running this test
      subject(:complete_game) { described_class.new(8, '9') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        neg_num = '-21'
        valid_num = '8'
        allow(game_loop).to receive(:player_input).and_return(big_num, neg_num, valid_num)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game) { described_class.new(nil, nil, 5) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        empathy_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game.final_message }.to output(empathy_phrase).to_stdout
      end
    end
  end
end
