...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    subject(:test_game) {described_class.new(100)}
    context 'when the previous step is 107' do
      it 'returns 7' do
        previous_step = 107
  ...
  describe '#play' do
    context 'when the random number is 60' do
      subject(:test_game) {described_class.new(60)}
    ...
    context 'when the random number is 40' do
      subject(:test_game) { described_class.new(40) }
      # remove the 'x' before running this test
      it 'will return 7' do
    ...
    context 'when the random number is 99' do
      subject(:test_game) { described_class.new(99) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = test_game.play
        expect(result).to eq(7)
