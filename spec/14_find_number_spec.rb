...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:guessing) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(guessing.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:guessing) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(guessing.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:guessing) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(guessing.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:guessing) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(guessing.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:generated_num) { double('generated_num') }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:ending_game) { described_class.new(0, 10, generated_num, 7) }
      ...
      it 'is game over' do
        allow(generated_num).to receive(:value).and_return(7)
    ...
    context 'when guess and random_number are not equal' do
      subject(:end_game) { described_class.new(0, 10, generated_num, 2) }
      it 'is not game over' do
        allow(generated_num).to receive(:value).and_return(9)
