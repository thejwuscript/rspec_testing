...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        incorrect = '&%'
        correct = '7'
        allow(game_input).to receive(:gets).and_return(incorrect, correct)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        incorrect2 = 'a'
        correct = '7'
        allow(game_input).to receive(:gets).and_return(incorrect1, incorrect2, correct)
      end
  ...
  describe '#verify_input' do
    ...
    subject(:test_input) { described_class.new(10, 100) }
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        input = 60
        expect(test_input.verify_input(min, max, input)).to be(60)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = test_input.instance_variable_get(:@maximum)
        player_input = 200
        result = test_input.verify_input(min, max, player_input)
  ...
  describe '#display_turn_order' do
    ...
    subject(:search_game) { described_class.new(1, 10) }
    let(:binary_tree) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(binary_tree).to receive(:make_guess).and_return(5)
      allow(search_game).to receive(:display_guess)
      allow(binary_tree).to receive(:update_range)
      allow(binary_tree).to receive(:game_over?)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      search_game.display_turn_order(binary_tree)
      guess_count = search_game.instance_variable_get(:@guess_count)
      expect(guess_count).to be(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_tree).to receive(:make_guess).once
      search_game.display_turn_order(binary_tree)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary_tree).to receive(:update_range).once
      search_game.display_turn_order(binary_tree)
    end
